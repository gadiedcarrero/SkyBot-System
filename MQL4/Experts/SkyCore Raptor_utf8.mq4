//+------------------------------------------------------------------+
//|                                                SkyCoreRaptor.mq4 |
//|                                    Copyright 2024, TradeTechCorp |
//|                                        http://tradetechcorp.com/ |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//|                  SKYCORE RAPTOR - INPUT CONFIGURATION            |
//+------------------------------------------------------------------+

//═══════════════════════════════════════════════════════════════════
//  SKYCORE RAPTOR v2.0 - Triple Martingale Recovery System
//═══════════════════════════════════════════════════════════════════
...
extern int MaxSpread = 90; // Maximum allowed spread (in points)

// Internal variables
string PastMaxMin = "Solo_MaxMin_Ind";
int CantHorasEspera = 5;
bool NuevaOperacion = false;
// Internal variables (non-input)
double metaDiaria = 0;
int CantOperacionesMax = 1;
...
input bool usarMegaComoGatillo = true; // Use MEGA signal as main entry trigger
input bool banderaEstocastico = false; // Require Stochastic confirmation
input bool restriccionMedia200 = false; // Restrict entries based on MA200 direction
...
// Dynamic TP/SL (Recommended - automatically adapts to market volatility)
input bool calculoTPSLAutomatico = true; // Use dynamic TP/SL based on bar height
input int barrasParaPromedio = 20; // Number of bars for average calculation
input double slMultiplicador = 3.0; // SL = X times average bar height
input double tpMultiplicador = 1.0; // TP = X times average bar height

input string __RISK_NOTE__ = "⚠ If dynamic TP/SL is ON, fixed values below are IGNORED";

// Manual TP/SL (only used if calculoTPSLAutomatico = false)
...
int OSLevel = 50;
bool usarIA = false;
double tpPercent = 1.5;
double slPercent = 0.5;
int VelasContra = 8;
double DojiRatio = 0.25;
double RechazoMinimo = 1.1;
double extraSlSpace = 0;
int countTS = 102;
double datatrailingStopPercentage = 0.5;
double N = 1;
double myVela = 1;

// Position Sizing
double lotajeDeseado = 0.01;

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  4. POSITION SIZING (Compound Interest)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __SIZING__ = "════ POSITION SIZING ════";
input double capitalBase = 3000.0; // Balance required for 0.01 lots (Conservative: 5000 | Aggressive: 3000)
input double lotsPorCapital = 0.01; // Lot size per capital unit

// Internal
bool useInteresCompuesto = true;

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  5. MARTINGALE RECOVERY SYSTEM (Raptor: 3 Attempts)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
input bool hacerMartingalaAcumulativo = false; // Calculate lot size to cover accumulated losses
input int maxLost = 100; // Maximum consecutive losses before stopping

// Martingale Type (Internal - Do not change)
input int tipoMartingala = 1; // 0=Classic | 1=Bad Streak System

// Internal variables
bool martingala = true;
int cuandoUsarMartingala = 1;
double porcentajeExtra = 0.1;
bool usarRestriccionVelaGrande = true;

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  6. EXIT STRATEGY
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __EXIT__ = "════ EXIT STRATEGY ════";

// Trailing Stop
input bool useTS = true; // Enable Trailing Stop
input double minDistanceToActivateTS = 10000.0; // Minimum profit (points) to activate TS
input double trailingStopBuffer = 6000.0; // Trailing distance (points)
input double initialTSMove = 6000.0; // Initial SL move when TS activates

// Breakeven
input bool useBreakevenStop = true; // Move SL to breakeven when in profit
input double breakevenDistance = 3000.0; // Profit distance (points) to trigger breakeven
input double breakevenLossPips = 1000.0; // Small loss allowed at breakeven (points)

// Additional Exit Signals
input bool useMA200Exit = false; // Close trade if candle closes beyond MA200

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  7. SCALPING & QUICK EXITS
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __SCALPING__ = "════ SCALPING MODE ════";
input bool usarScalpingAgresivo = false; // Enable aggressive scalping (close at small profit)
input int gananciaMinimaTics = 3000; // Minimum profit in ticks to close
input int gananciaRapidaTics = 1500; // Quick profit target in ticks

// Internal
bool cerrarEnPrimeraGanancia = true;

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  8. DRAWDOWN PROTECTION
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __PROTECTION__ = "════ DRAWDOWN PROTECTION ════";
input bool usarProteccionDrawdown = true; // Stop trading if drawdown exceeds limit
input double maxDrawdownPorcentaje = 50.0; // Maximum drawdown % allowed

// Internal
bool cerrarSoloPerdedoras = true;
int slippage = 3;

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  9. MARKET SAFETY FILTERS (IsMarketSafe System)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __FILTERS__ = "════ SAFETY FILTERS ════";

// Enable/Disable Individual Filters
input bool usarFiltroEMATrenzada = true; // Block if EMAs are entangled
input bool usarFiltroRangoAdaptativo = true; // Block if range is too small
input bool usarFiltroPrecioPegadoEMA = true; // Block if price stuck to EMA
input bool usarFiltroSlopeEMA = true; // Block if EMA slope too flat
input bool usarFiltroAnguloEMA = true; // Block if EMA angle insufficient
input bool usarFiltroCanalEstrecho = true; // Block if price in narrow channel
input bool usarFiltroAlternanciaVelas = true; // Block if alternating candles
input bool usarFiltroMechasLargas = true; // Block if long wicks present
input bool usarFiltroCuerpoMinimo = true; // Block if tiny candle bodies
input bool usarFiltroFaltaContinuacion = true; // Block if no continuation
input bool usarFiltroRetrocesoInmediato = true; // Block if immediate retracement
input bool usarFiltroAceleracionReal = true; // Block if no real acceleration
input bool detectarRangoHorizontal = true; // Detect ranging markets

// Filter Sensitivity Parameters (Advanced)
input int filtroLookback = 20; // Bars to analyze
input bool mostrarDebugFiltros = false; // Show filter debug messages

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  10. TIME MANAGEMENT
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __TIME__ = "════ TIME FILTERS ════";

// Dangerous Hours Filter
input bool evitarHorariosPeligrosos = true; // Avoid historically bad trading hours
input int offsetBrokerGMT = 2; // Broker GMT offset (e.g., GMT+2 = 2)
input int horaPeligrosa1Inicio = 17; // Block period 1 start (broker time)
input int horaPeligrosa1Fin = 20; // Block period 1 end
input int horaPeligrosa2Inicio = 9; // Block period 2 start
input int horaPeligrosa2Fin = 10; // Block period 2 end

// Friday Protection
input bool bloquearViernesCaotico = true; // Block Friday volatile hours
input int horaInicioViernes = 12; // Friday block start hour
input int minutoInicioViernes = 30; // Friday block start minute
input int horaFinViernes = 24; // Friday block end hour

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  11. ADVANCED SETTINGS
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __ADVANCED__ = "════ ADVANCED (DO NOT MODIFY) ════";
input bool ajusteEntradaStochastic = false; // Experimental: Stochastic re-entry
input bool usarCierrePorEstocastico = false; // Close by Stochastic signal
...
bool usarFiltrosTendencia = true;
int filtrosMinimosRequeridos = 3;
bool filtrarPorMA50 = true;
bool filtrarPorRSI = true;
bool filtrarPorMACD = true;
bool filtrarPorADX = true;
bool filtrarPorEstructura = true;
bool filtrarPorVelocidadMA = true;
bool modoReactivo = true;
color colorLineaAlta = clrLime;
color colorLineaBaja = clrRed;
int grosorLinea = 2;
int filtroEMATrenzadaMaxCruces = 3;
double filtroRangoMinMultiplicadorATR = 1.8;
double filtroPrecioPegadoFactorATR = 0.30;
double filtroPrecioPegadoMaxRatio = 0.40;
double filtroSlopeMinFactorATR = 0.20;
double filtroAnguloMinGrados = 3.5;
double filtroCanalFactorATR = 0.35;
double filtroCanalMaxRatioCentral = 0.50;
double filtroAlternanciaMaxRatio = 0.45;
double filtroMechasFactorCuerpo = 2.0;
double filtroCuerpoMinFactorAltura = 0.25;
double filtroContinuacionMinFactorAlt = 0.30;
double filtroRetrocesoMaxFactorAlt = 0.40;
double filtroAceleracionMinFactorAlt = 0.20;
bool usarFiltroPendienteMA200 = true;
int pendienteLookbackBars = 20;
double pendienteMinPipsPorBarra = 0.05;
bool evitarRangoMA200 = true;
bool usarFiltroPendienteMA50 = true;
double pendienteMinPipsPorBarraMA50 = 0.08;
bool usarFiltroMA50 = true;
int periodoMA50 = 5;
string SeparadorEntradaMedias = "---Entrada por Alineación de Medias---";
bool usarEntradaPorMedias = true;
bool usarEstocastico = true;
int periodoMACorta = 50;
int periodoMALarga = 200;
int barrasParaTendencia = 5;
int periodoMAUltraCorta = 5;
...
// Variables para nueva estrategia de martingala
double lotajePerdidoGuardado = 0.0;  // Guarda el lotaje de la operación perdida
bool enRachaMala = false;
int intentosMartingalaConsecutivos = 0;  // Contador de intentos consecutivos de martingala (máximo 3)
double perdidasAcumuladas = 0.0;  // Acumula el monto en dolares de todas las perdidas             // Indica si estamos en racha mala (operando a 0.01)
int tipoOperacionGanadora = -1;              // Guarda el tipo de operación que ganó (OP_BUY=0, OP_SELL=1, -1=ninguna)
...
// Variable para fijar el SL Buy and Sell
double priceSL = 0;

// Variable para fijar el TP Buy and Sell
double priceTP = 0;
...
int OnInit()
  {
// ===== LOG DE PRUEBA - VERIFICAR COMPILACIÓN =====
// ===== FIN LOG DE PRUEBA =====
   ...
   if(usarNotificacionDia)
     {
      ...
      if(Weekend == true)
      ...
      else
        {
         ...
         if(Weekend == false && ContadordeBuys() == 0
            && ContadordeSells() == 0)
           {
            CrearOrdenes();
           }
...
// Cerrar por estocástico independientemente del estado de useTS
   if(conditionEnSobrecompra && conditionHasSells)
     {
      LogEnPantalla("Cerrando SELL porque el estocástico está en sobrecompra");
      CloseAllSells();
      ResetSellFlags();
     }
   else
...
//|                                                                  |
//+------------------------------------------------------------------+
void DrawHLineSL(double price)
  {
   ObjectDelete("lineSL");
   ObjectCreate(0, "lineSL", OBJ_HLINE, 0, 0, price);
...
//|                                                                  |
//+------------------------------------------------------------------+
void DrawHLineTP(double price)
  {
   ObjectDelete("lineTP");
   ObjectCreate(0, "lineTP", OBJ_HLINE, 0, 0, price);
   ...
   if(usarIA)
     {
      ...
      priceSL = NormalizeDouble(mySL - slPercent * averageRangeInPips * Point, Digits);
      priceSL -= additionalPips * Point;
     }
   else
      if(useSL == false)
        {
         double averageRangeInPips = AverageRangeInPips(High, Low, 0, 200) / Point;
         double additionalPips = averageRangeInPips * extraSlSpace;
         priceSL = NormalizeDouble(mySL, Digits);
         priceSL -= additionalPips * Point;
        }
      else
        {
         priceSL = NormalizeDouble(Bid - ObtenerSLDinamico() * Point, Digits);
        }
   DrawHLineSL(priceSL);
   ...
   if(usarIA)
     {
      ...
      if(useSL == false)
        ...
        {
         priceSL = NormalizeDouble(Ask + ObtenerSLDinamico() * Point, Digits);
        }
   DrawHLineSL(priceSL);
...
void CrearTPBuy(double orderOpenPrice)
  {
   ...
   double slDistance = orderOpenPrice - priceSL; // Calcula la distancia al SL
   if(usarIA)
     {
      useTP = false;
      priceTP = orderOpenPrice + tpPercent * slDistance; // Establece el TP al doble de la distancia al SL
     }
   else
      if(useTP == false)
        {
         priceTP = orderOpenPrice + N * slDistance; // Establece el TP a N veces la distancia al SL
        }
      else
        {
         priceTP = NormalizeDouble(Ask + ObtenerTPDinamico() * Point, Digits);
...
void CrearTPSell(double orderOpenPrice)
  {
   ...
   double slDistance = priceSL - orderOpenPrice; // Calcula la distancia al SL
   if(usarIA)
     {
      useTP = false;
      priceTP = orderOpenPrice - tpPercent * slDistance; // Establece el TP al doble de la distancia al SL
      ...
      if(useTP == false)
        ...
        {
         priceTP = NormalizeDouble(Bid - ObtenerTPDinamico() * Point, Digits);
        }
   DrawHLineTP(priceTP);
   ...
   if(Low[i] > High[i + 2] && MathAbs(Low[i] - High[i + 2]) / Point >= averageRangeInPips && AreThreeBullishCandles(Open, Close, i) && kStoch < OSLevel)
     {
      highAlcista = Low[i];
      lowAlcista = High[i + 2];
      LogEnPantalla("🟢 FVG ALCISTA detectado en zona baja - Stoch: " + DoubleToStr(kStoch, 2));
     }
   ...
   if(High[i] < Low[i + 2] && MathAbs(Low[i + 2] - High[i]) / Point >= averageRangeInPips && AreThreeBearishCandles(Open, Close, i) && kStoch > OBLevel)
     {
      highBajista = High[i];
      lowBajista = Low[i + 2];
      LogEnPantalla("🔴 FVG BAJISTA detectado en zona alta - Stoch: " + DoubleToStr(kStoch, 2));
     }
               ...
               else
                 {
                  if(!martingala)
                  ...
                  else
                    {
                     ...
                     double lotsCalculado = prevLots * myMultiplier;
                     lotsCalculado = NormalizeDouble(lotsCalculado / lotstep, 0) * lotstep;
                     if(lotsCalculado < minlot)
                        lotsCalculado = minlot;
                     if(lotsCalculado > MaximalLots)
...
// ===== MARTINGALA NUEVA (RACHA MALA) =====
     ...
     {
      // Revisar el historial sin modificar contadores ni variables globales
      for(int x = OrdersHistoryTotal() - 1; x >= 0; x--)
        {
         if(OrderSelect(x, SELECT_BY_POS, MODE_HISTORY))
           {
               ...
               if(OrderProfit() > 0)
                 {
                  if(enRachaMala && prevLots <= lotajeMinimo)
                    {
                     // NUEVO: Guardamos el tipo de operación que ganó
                     tipoOperacionGanadora = OrderType();

                     // NUEVO: NO aplicamos multiplicador inmediatamente
                     // Esperamos a que la siguiente operación sea del mismo tipo
                     if(lotajePerdidoGuardado > 0)
                       {
                        LogEnPantalla("✓ Ganó en racha mala - Esperando confirmación de tendencia");

                        // CALCULAR PRÓXIMO LOTAJE DE RECUPERACIÓN PARA DASHBOARD
                        if(hacerMartingalaAcumulativo && perdidasAcumuladas > 0)
                          {
                           ...
                           double lotsCalculado = (perdidasAcumuladas / ganancia001) * lotajeMinimo * myMultiplier;
...
// === LOG DE PRUEBA - VERIFICAR QUE SE EJECUTA CÓDIGO NUEVO ===
   ...
   contadorLlamadas++;
   if(contadorLlamadas % 10 == 1) // Solo cada 10 llamadas para no saturar
     {
     }
// === FIN LOG DE PRUEBA ===
...
// Control de Pérdidas Máximas
   if(contadorPerdidas() > maxLost)
     {
      lots = lotajeBaseActual;
      lotajePerdidoGuardado = 0.0;
      enRachaMala = false;
      perdidasAcumuladas = 0.0;  // NUEVO: Reset perdidas acumuladas
      intentosMartingalaConsecutivos = 0; // Resetear contador de intentos
      LogEnPantalla("Pérdidas máximas alcanzadas. Reset completo. Lotaje: "+ DoubleToString(lots, 2));
...
// Revisión del Historial de Órdenes
   for(int x = OrdersHistoryTotal() - 1; x >= 0; x--)
     {
      if(OrderSelect(x, SELECT_BY_POS, MODE_HISTORY))
        {
            ...
            if(OrderProfit() > 0)
              {
               ...
               if(enRachaMala && prevLots <= lotajeMinimo)
                 {
                  // === LOG DE PRUEBA - ENTRANDO EN RACHA MALA ===
                  // === FIN LOG DE PRUEBA ===

                  // IMPORTANTE: La racha mala NO depende del tipo de operación
                  // Salimos de racha mala y aplicaremos martingala en la próxima operación (sin importar BUY o SELL)
                  if(lotajePerdidoGuardado > 0)
                    {
                     LogEnPantalla("✓ Ganó en racha mala - Próxima operación aplicará martingala");

                     // Salimos de racha mala - la próxima operación (BUY o SELL) aplicará martingala
                     enRachaMala = false;
                     // NO guardamos tipoOperacionGanadora - no importa el tipo
                     tipoOperacionGanadora = -1; // Mantener en -1 para indicar que no importa el tipo

                     // Seguimos con lotaje mínimo por ahora, próxima decisión en Compra()/Venta()
                     lots = lotajeMinimo;
                    }
                  else
                    {
                 ...
                 {
                  // === LOG DE PRUEBA - NO ENTRÓ EN RACHA MALA ===
                  // === FIN LOG DE PRUEBA ===

                  // Si ganó con lotaje alto durante intentos de recuperación
                  // CORREGIDO: Verificar si estamos en recuperación (lotajePerdidoGuardado > 0) en lugar de solo el contador
                  if(prevLots > lotajeBaseActual && lotajePerdidoGuardado > 0 && !enRachaMala)
                    {
                     Print("🟢 DEBUG RECUPERACIÓN GANADORA | prevLots:", prevLots, " | lotajeBase:", lotajeBaseActual,
                           " | lotajePerdidoGuardado:", lotajePerdidoGuardado, " | intentos:", intentosMartingalaConsecutivos);

                     // Verificar si la ganancia cubre todas las pérdidas acumuladas
                     double gananciaActual = OrderProfit() + OrderSwap() + OrderCommission();

                     // Si hay pérdidas acumuladas y la ganancia no las cubre completamente, continuar con martingala
                     if(perdidasAcumuladas > 0 && gananciaActual < perdidasAcumuladas)
                       {
                        // NO resetear - mantener variables para continuar con martingala
                        // El siguiente trade aplicará martingala si pierde, o continuará si gana
                        lots = lotajeBaseActual; // Usar lotaje base para próxima operación
                        // Mantener lotajePerdidoGuardado y perdidasAcumuladas
                       }
                     else
                        if(perdidasAcumuladas > 0 && gananciaActual >= perdidasAcumuladas)
                          {
                           // La ganancia cubre todas las pérdidas - Resetear
                           lots = lotajeBaseActual;
                           lotajePerdidoGuardado = 0.0;
                           enRachaMala = false;
                           ...
                           tipoOperacionGanadora = -1;
                           intentosMartingalaConsecutivos = 0; // Resetear contador de intentos
                           LogEnPantalla("✅ Recuperación exitosa - Volviendo a interés compuesto");
                          }
                        else
                          {
                           // No hay pérdidas acumuladas (martingala no acumulativa)
                           // Ganar con lotaje alto durante recuperación significa que recuperamos exitosamente
                           // RESETEAR a lotaje base - la martingala solo multiplica después de PÉRDIDAS, no después de GANANCIAS
                           // RESETEAR: Ganar con lotaje alto significa recuperación exitosa - volver a lotaje base
                           lotajePerdidoGuardado = 0.0;
                           lots = lotajeBaseActual; // Usar lotaje base para próxima operación
                           enRachaMala = false; // Asegurar que no estamos en racha mala
                           intentosMartingalaConsecutivos = 0; // Resetear contador de intentos porque ganó
                           tipoOperacionGanadora = -1; // Resetear tipo ganadora
                          }
                    }
                  // Si ganó con lotaje igual al base pero estábamos en recuperación
                  else
                     if(prevLots >= lotajeBaseActual && lotajePerdidoGuardado > 0)
                       {
                        // Verificar si hay pérdidas acumuladas
                        if(perdidasAcumuladas > 0)
                          {
                           double gananciaActual = OrderProfit() + OrderSwap() + OrderCommission();
                           if(gananciaActual < perdidasAcumuladas)
                             {
                              lots = lotajeBaseActual;
                              // Mantener variables de recuperación
                             }
                           else
                             {
                              lots = lotajeBaseActual;
                              ...
                              perdidasAcumuladas = 0.0;
                              tipoOperacionGanadora = -1;
                              intentosMartingalaConsecutivos = 0;
                             }
                          }
                        else
                          {
                           // Sin pérdidas acumuladas (martingala no acumulativa)
                           // Ganar con lotaje base durante recuperación significa que recuperamos exitosamente
                           // RESETEAR a lotaje base - la martingala solo multiplica después de PÉRDIDAS, no después de GANANCIAS
                           // RESETEAR: Ganar con lotaje base significa recuperación exitosa - volver a lotaje base
                           // PERO si esta es la operaciÃ³n que ganÃ³ en racha mala O es la primera martingala,
                           // NO resetear porque todavÃ­a no hemos completado el ciclo de recuperaciÃ³n
                           // CORREGIDO: Verificar si estamos en recuperación en lugar de solo el contador
                           if(prevLots > lotajeMinimo && lotajePerdidoGuardado > 0 && !enRachaMala)
                             {
                              ...
                              intentosMartingalaConsecutivos = 0; // Resetear contador de intentos porque ganó
                              tipoOperacionGanadora = -1; // Resetear tipo ganadora
                              LogEnPantalla("✅ Recuperación exitosa (sin acumulativo) - Volviendo a interés compuesto");
                             }
                           else
                             {
                              // Esta es la operaciÃ³n que ganÃ³ en racha mala - NO resetear
                              lots = lotajeBaseActual;
                             }
                          }
                       }
                       ...
                       {
                        // Ganancia normal - Resetear solo si no hay recuperación en curso
                        if(lotajePerdidoGuardado == 0 && perdidasAcumuladas == 0)
                          {
                           lots = lotajeBaseActual;
                           enRachaMala = false;
                           tipoOperacionGanadora = -1;
               ...
               // IMPORTANTE: Retornar inmediatamente después de procesar la orden más reciente
               return lots; // Retornar inmediatamente después de procesar la orden más reciente
              }
            // CASO 2: Operación PERDEDORA
            else
              {
               if(!martingala)
                 {
                  lots = lotajeBaseActual;
                  LogEnPantalla("Martingala desactivada. Lotaje: "+ DoubleToString(lots, 2));
                 }
               else
                 {
                  // Si acabamos de perder con martingala aplicado (lote mayor al base)
                  // Esto significa que ya salimos de racha mala y estamos intentando recuperar
                  // IMPORTANTE: SOLO entrar si NO estamos en racha mala
                  if(prevLots > lotajeBaseActual && !enRachaMala)
                    {
                     // Verificar si tenemos lotaje perdido guardado (indica que estamos en proceso de recuperación)
                     if(lotajePerdidoGuardado > 0)
                       {
                        // === LOG DE PRUEBA - PÉRDIDA CON MARTINGALA ===
                        // === FIN LOG DE PRUEBA ===

                        // Incrementar contador de intentos consecutivos
                        intentosMartingalaConsecutivos++;
                        ...
                        // Si ya hicimos 3 intentos consecutivos, entrar en racha mala
                        if(intentosMartingalaConsecutivos >= 3)
                          {
                           // Guardamos este lotaje perdido y entramos en racha mala
                           lotajePerdidoGuardado = prevLots;
                           enRachaMala = true;
                           intentosMartingalaConsecutivos = 0; // Resetear contador
                           tipoOperacionGanadora = -1; // Resetear tipo ganadora

                           // === LOG DE PRUEBA - CONFIRMACIÓN ===
                           // === FIN LOG ===

                           lots = lotajeMinimo;
                           LogEnPantalla("Perdió 3 intentos consecutivos. Guardando lotaje: "+ DoubleToString(prevLots, 2) +
                                         ". Entrando en racha mala con: "+ DoubleToString(lots, 2));
                          }
                        else
                          {
                           // Aún tenemos intentos disponibles, aplicar martingala multiplicando el lotaje

                           // IMPORTANTE: Actualizar lotajePerdidoGuardado al lotaje que acabamos de perder
                           // para que las funciones Compra()/Venta() usen el lotaje correcto
                           lotajePerdidoGuardado = prevLots;

                           double nuevoLotaje = prevLots * myMultiplier;
                           nuevoLotaje = NormalizeDouble(nuevoLotaje / lotstep, 0) * lotstep;
                           if(nuevoLotaje < minlot)
                              nuevoLotaje = minlot;
                           if(nuevoLotaje > MaximalLots)
                              nuevoLotaje = MaximalLots;

                           lots = nuevoLotaje;
                           LogEnPantalla("Intento " + IntegerToString(intentosMartingalaConsecutivos) + "/3. Aplicando martingala: " +
                                         DoubleToString(prevLots, 2) + " -> " + DoubleToString(lots, 2));
                          }
                       }
                     else
                       {
                        // No hay lotaje perdido guardado, pero el lotaje es mayor al base
                        // Esto no debería pasar normalmente, pero por seguridad vamos a racha mala
                        lotajePerdidoGuardado = prevLots;
                        enRachaMala = true;
                        intentosMartingalaConsecutivos = 0;
                        tipoOperacionGanadora = -1;
                        lots = lotajeMinimo;
                        LogEnPantalla("Pérdida inesperada. Entrando en racha mala con: "+ DoubleToString(lots, 2));
                       }
                    }
                  // Si estamos en racha mala (operando a 0.01)
                  else
                     if(enRachaMala)
                       {
                        // Seguimos operando con lotaje mínimo
                        lots = lotajeMinimo;
                        LogEnPantalla("En racha mala. Operando con: "+ DoubleToString(lots, 2));
                       }
                     // Primera pérdida con lotaje normal (interés compuesto) - va directo a racha mala
                     else
                       {
                        // Guardamos el lotaje perdido y entramos en racha mala
                        lotajePerdidoGuardado = prevLots;
                        enRachaMala = true;
                        intentosMartingalaConsecutivos = 0; // Resetear contador (aún no empezamos recuperación)
                        tipoOperacionGanadora = -1; // Resetear tipo ganadora
                        ...
                        lots = lotajeMinimo;
                        LogEnPantalla("Primera pérdida con interés compuesto. Guardando lotaje: "+ DoubleToString(prevLots, 2) +
                                      ". Entrando en racha mala con: "+ DoubleToString(lots, 2));
                       }
                 }
   ...
   if(lots == 0.0)
     {
      ...
      intentosMartingalaConsecutivos = 0; // Resetear contador de intentos
      perdidasAcumuladas = 0.0;  // NUEVO: Reset perdidas acumuladas
      LogEnPantalla("Primera orden. Lotaje: "+ DoubleToString(lots, 2));
     }
            ...
            if(profitTics >= gananciaMinimaTics && profit > 0)
              {
               ...
               if(closed)
                 {
                  LogEnPantalla("🎯 SCALPING: Cerrado con " + DoubleToString(profitTics, 0) + " tics de ganancia. Profit: $" + DoubleToString(profit, 2));
                  ...
                  if(!enRachaMala)
                    {
                     // No está en racha mala, reset completo
                     lotajePerdidoGuardado = 0.0;
                     perdidasAcumuladas = 0.0;
                     tipoOperacionGanadora = -1;
                    }
...
//+------------------------------------------------------------------+
double SeleccionarMartingala()
  {
   if(tipoMartingala == 0)
      return MartingalaClasica();
   else
      return MyMartingala();
  }
...
//+------------------------------------------------------------------+
double MartingalaClasica()
  {
   static int numeroDeIntentos = 0;
   ...
   if(contadorPerdidas() > maxLost)
     {
      lots = lotajeBaseActual;
      numeroDeIntentos = 0;
      LogEnPantalla("Pérdidas máximas alcanzadas (Clásica). Reset. Lotaje: "+ DoubleToString(lots, 2));
      lots = NormalizeDouble(lots / lotstep, 0) * lotstep;
      if(lots < minlot)
         lots = minlot;
      if(lots > MaximalLots)
            ...
            if(OrderProfit() > 0)
              {
               ...
               lots = lotajeBaseActual;
               numeroDeIntentos = 0;
               LogEnPantalla("Ganancia (Clásica). Reset. Lotaje: "+ DoubleToString(lots, 2));
            ...
            // CASO 2: Operación PERDEDORA
            else
              {
               if(!martingala)
                 {
                  lots = lotajeBaseActual;
                  LogEnPantalla("Martingala desactivada (Clásica). Lotaje: "+ DoubleToString(lots, 2));
                 }
               else
                 {
                  ...
                  if(numeroDeIntentos < maxIntentos)
                    {
                     numeroDeIntentos++;
                     lots = prevLots * myMultiplier;
                     LogEnPantalla("Pérdida (Clásica). Multiplicando: "+ DoubleToString(prevLots, 2) + " x " + DoubleToString(myMultiplier, 2) + " = " + DoubleToString(lots, 2));
                    }
                  else
                    {
                     ...
                     lots = lotajeBaseActual;
                     numeroDeIntentos = 0;
                     LogEnPantalla("Máx intentos alcanzados (Clásica). Reset. Lotaje: "+ DoubleToString(lots, 2));
                    }
                 }
   ...
   if(lots == 0.0)
     {
      lots = lotajeBaseActual;
      numeroDeIntentos = 0;
      LogEnPantalla("Primera orden (Clásica). Lotaje: "+ DoubleToString(lots, 2));
     }
...
bool FiltroEMATrenzada(int lookback, int maxCruces)
{
   ...
   for(int i=1; i<lookback; i++)
   {
      ...
      if(crossUp || crossDown) crossCount++;
   }
...
bool IsMarketSafe()
{
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroEMATrenzada");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroRangoAdaptativo");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroPrecioPegadoEMA");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroSlopeEMA");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroAnguloEMA");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroCanalEstrecho");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroAlternanciaVelas");
      return(false);
   }
   ...
   if(usarFiltroMechasLargas &&
      !FiltroMechasLargas())
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroMechasLargas");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroCuerpoMinimo");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroFaltaContinuacion");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroRetrocesoInmediato");
      return(false);
   }
   ...
   {
      if(mostrarDebugFiltros) Print("IsMarketSafe: Bloqueado por FiltroAceleracionReal");
      return(false);
   }
...
// ====== Stochastic Close/Close (línea principal %K) ======
   double kNow  = iStochastic(NULL, 0, 8, 3, 3, MODE_SMA, 1, MODE_MAIN, 1);
   double kPrev = iStochastic(NULL, 0, 8, 3, 3, MODE_SMA, 1, MODE_MAIN, 2);
...
// ===== Filtros extra que ya usabas =====
   condicionBuyMedia  = restriccionMedia200 ? (media200 < mediaPrice) : true;  // BUY encima MA200 (si restricción activa)
   condicionSellMedia = restriccionMedia200 ? (media200 > mediaPrice) : true;  // SELL debajo MA200 (si restricción activa)
...
// Debug: Mostrar estado de FVG cada tick
   ...
   debugCounter++;
   if(debugCounter % 100 == 0)   // Cada 100 ticks
     {
     }
   ...
   if(usarEntradaPorMedias)
     {
      ...
      // Condición adicional: Estocástico (camino completo como en OptimusExtreme)
      ...
      // La pendiente del Stochastic es más efectiva que las zonas

      // Calcular pendiente del Stochastic (ya calculado arriba como stochNow y stochPrev)
      ...
      // Verde = Stochastic subiendo (pendiente positiva)
      // Roja  = Stochastic bajando (pendiente negativa)
      // Gris  = Stochastic plano (sin cambio)

      if(stochSubiendo && stochNow < OSLevel)  // BUY: zona baja (0-50) + pendiente positiva
        {
         G_stochBuyFlag = true;   // SCHO Verde - Stochastic subiendo en zona baja
         G_stochSellFlag = false;
        }
      else
         if(stochBajando && stochNow > OBLevel)  // SELL: zona alta (50-100) + pendiente negativa
           {
            G_stochBuyFlag = false;
            G_stochSellFlag = true;  // SCHO Rojo - Stochastic bajando
           }
      ...
      // La MEGA-BANDERA se actualiza en TIEMPO REAL según la alineación de las tres medias
      // Se activa cuando las 3 medias tienen la misma tendencia
      // Se desactiva INMEDIATAMENTE cuando cualquiera de las 3 cambia de tendencia

      // MEGA-BUY: Activar solo si las TRES medias son alcistas
      if(mediasAlcistas && precioEncimaMA200)
        {
         G_megaBuyFlag = true;  // Las 3 medias alineadas alcista
        }
      else
        {
         // Si una de las tres medias ya NO es alcista, desactivar MEGA-BUY inmediatamente
         if(G_megaBuyFlag)  // Solo log si cambia de estado
           {
      ...
      if(mediasBajistas && precioDebajoMA200)
        {
         G_megaSellFlag = true;  // Las 3 medias alineadas bajista
        }
      else
        {
         // Si una de las tres medias ya NO es bajista, desactivar MEGA-SELL inmediatamente
         if(G_megaSellFlag)  // Solo log si cambia de estado
           {
      ...
      // ===== NUEVO: Gatillo basado en CRUCE MA8 x MA5 =====
      ...
      // NO usar estas variables, ya están desactivadas
      /*
      // Actualizar visualización de la bandera del Stochastic
      static string ultimoTipoBandera = "";
      string tipoActual = "";
      bool mostrarBandera = true;

      // Mostrar el estado de la mega-bandera
      if(megaBuyAutorizado)
         {
          tipoActual = "MEGABUY"; // Mega-bandera verde (todo alineado para compra)
          mostrarBandera = true;
         }
      else if(megaSellAutorizado)
         {
          tipoActual = "MEGASELL"; // Mega-bandera roja (todo alineado para venta)
          mostrarBandera = true;
         }
      else if(G_stochBuyFlag)
         {
          tipoActual = "BUY"; // Verde (solo Stochastic)
          mostrarBandera = true;
         }
      else if(G_stochSellFlag)
         {
          tipoActual = "SELL"; // Rojo (solo Stochastic)
          mostrarBandera = true;
         }
      else
         {
          tipoActual = "NONE"; // Gris (neutro)
          mostrarBandera = true;
         }
      */

      // NO mostrar el cuadrito SCHO - solo MEGA y medias
      /*
      if(tipoActual != ultimoTipoBandera)
        {
         if(mostrarBandera)
           {
            MostrarBanderaStoch(true, tipoActual); // Mostrar bandera
           }
         ultimoTipoBandera = tipoActual;
        }
      */
      ...
      Print("IsMarketSafe=", IsMarketSafe());
      ...
      logMediasCounter++;
      if(logMediasCounter % 100 == 0) // Cada 100 ticks
        {
        }
     }
   else
     {
      // Sistema original (FVG + Stoch + filtros)
      enterUp   = (CondicionBuy  && G_readyBuy && condicionBuyMedia && restriccionVela && filtrosBuyOK && pendienteOkBuy && tendenciaCortaOkBuy);
      enterDown = (CondicionSell && G_readySell && condicionSellMedia && restriccionVela && filtrosSellOK && pendienteOkSell && tendenciaCortaOkSell);
     }

// Debug detallado de condiciones de entrada
   static int debugEntradaCounter = 0;
   debugEntradaCounter++;
   if(debugEntradaCounter % 50 == 0)   // Cada 50 ticks
     {
     }
...
// ====== COMPRA ======
   if(timeofOperation < restatiempo && enterUp)
     {
      // ⚠️ CONDICIÓN INAMOVIBLE: Verificar precio vs MA200 antes de ejecutar BUY
      double precioFinalBuy = iClose(NULL, 0, 0);
      double ma200FinalBuy = iMA(NULL, 0, periodoMALarga, 0, MODE_SMA, PRICE_CLOSE, 0);

      if(precioFinalBuy <= ma200FinalBuy)
        {
         LogEnPantalla("❌ BUY BLOQUEADO - Precio por debajo de MA200");
         return; // Salir sin ejecutar la orden
        }
...
// ====== VENTA ======
   if(timeofOperation < restatiempo && enterDown)
     {
      // ⚠️ CONDICIÓN INAMOVIBLE: Verificar precio vs MA200 antes de ejecutar SELL
      double precioFinalSell = iClose(NULL, 0, 0);
      double ma200FinalSell = iMA(NULL, 0, periodoMALarga, 0, MODE_SMA, PRICE_CLOSE, 0);

      if(precioFinalSell >= ma200FinalSell)
        {
         LogEnPantalla("❌ SELL BLOQUEADO - Precio por encima de MA200");
         return; // Salir sin ejecutar la orden
        }
...
//|                                                                  |
//+------------------------------------------------------------------+
void cerrarContrariasPorMedia200()
  {
   double precioActual = iClose(Symbol(), 0, 0);
   double media200 = iMA(Symbol(), 0, 200, 0, MODE_EMA, PRICE_CLOSE, 0);

   int cantidadBuys = ContadordeBuys();
   int cantidadSells = ContadordeSells();


   if(cantidadBuys >= 1 && precioActual < media200)
     {
      LogEnPantalla("Cerrando BUYs porque el precio cayó por debajo de la media 200");
      CloseAllBuys();
     }

   if(cantidadSells >= 1 && precioActual > media200)
     {
      LogEnPantalla("Cerrando SELLs porque el precio superó la media 200");
      CloseAllSells();
     }
...
// Inicializar lotaje con el valor calculado por la martingala
   lots = SeleccionarMartingala();

// IMPORTANTE: Si salimos de racha mala, sincronizar AMBOS lotajes
// para que la martingala se aplique independientemente del tipo (BUY o SELL)
   if(!enRachaMala && lotajePerdidoGuardado > 0)
     {
      // Sincronizacion eliminada - ahora usa variable unificada 'lots'
     }

// DEBUG: Estado de martingala antes de verificar


// IMPORTANTE: La racha mala NO depende del tipo de operación
// Si salió de racha mala (enRachaMala == false) y tiene lotaje perdido guardado, aplicar martingala
// PERO solo si MyMartingala() no ya calculó un lotaje mayor (indicando que perdió con martingala aplicada)
   double lotajeBaseActual = CalcularLotajeBase();
// Verificar si MyMartingala() ya calculó un lotaje mayor (cuando perdió con martingala aplicada)
// IMPORTANTE: MyMartingala() devuelve lots, así que verificamos lots
   bool yaCalculoMartingala = (lots > lotajeBaseActual);
// Verificar si MyMartingala() ya devolvió un lotaje mayor que el base (indicando que ya aplicó martingala)
// O si estamos en recuperación y necesitamos aplicar martingala
   bool necesitaAplicarMartingala = (!enRachaMala && lotajePerdidoGuardado > 0 && !yaCalculoMartingala);
   if(necesitaAplicarMartingala)
     {
      ...
      // Aplicar martingala sin verificar el tipo de operación

      // MODO ACUMULATIVO: Calcular lotaje que cubra perdidas acumuladas
      if(hacerMartingalaAcumulativo && perdidasAcumuladas > 0)
        {

         // Buscar la ganancia de la operación con 0.01 (sin importar tipo BUY o SELL)
         ...
         for(int x = OrdersHistoryTotal() - 1; x >= 0; x--)
           {
            if(OrderSelect(x, SELECT_BY_POS, MODE_HISTORY))
              {
               if(OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber &&
                  OrderLots() <= lotajeMinimo && OrderProfit() > 0)
                 {
         ...
         double perdidaNeta = perdidasAcumuladas - gananciaOperacion001;
         ...
         if(perdidaNeta > 0)
           {
            ...
            double lotajeNecesario = (perdidasAcumuladas / gananciaPromedioPor01Lote) * lotajeMinimo * myMultiplier;


            lots = NormalizeDouble(lotajeNecesario / lotstep, 0) * lotstep;
            // IMPORTANTE: Salimos de racha mala al aplicar martingala
            enRachaMala = false;
            // NO resetear intentosMartingalaConsecutivos - MyMartingala() lo controla
           }
         else
           {
      ...
      else
        {
         ...
         lots = lotajePerdidoGuardado * myMultiplier;
         // IMPORTANTE: Ya salimos de racha mala cuando ganó con 0.01
         // NO resetear intentosMartingalaConsecutivos - MyMartingala() lo controla
        }
      ...
      LogEnPantalla("✓✓ Aplicando martingala BUY - Lotaje: " + DoubleToString(lots, 2));
...
// GATING DE PRECIO: solo ejecutar BUY si el precio va MÁS ALTO que la última operación (detección de tendencia alcista)
   double candidateBuyPrice = NormalizeDouble(Ask, Digits);
   string gvLastExecKey = "OHL_LAST_EXEC_PRICE_" + Symbol() + "_" + IntegerToString(MagicNumber);
   double lastExecPrice = 0.0;
   if(GlobalVariableCheck(gvLastExecKey))
      lastExecPrice = GlobalVariableGet(gvLastExecKey);

// BUY debe ser MÁS ALTO que la última operación (sea BUY o SELL)
   bool buyAllowed = (lastExecPrice == 0.0) || (candidateBuyPrice > lastExecPrice);
   LogEnPantalla(">>> DEBUG BUY: Precio actual=" + DoubleToString(candidateBuyPrice, Digits) + " | Ultimo precio=" + DoubleToString(lastExecPrice, Digits) + " | Permitido=" + (buyAllowed ? "SI" : "NO (FANTASMA)"));

// NUEVO: Si ya hubo una orden fantasma BUY en este ciclo, bloquear hasta que se resetee la MEGA-BANDERA
   if(G_phantomBuyDetected && G_megaBuyFlag)
     {
      LogEnPantalla("⛔ BUY bloqueado - Esperando nuevo ciclo MEGA");
      return(false);
     }
   ...
   if(!buyAllowed)
     {
      ...
      // MARCAR que se detectó una orden fantasma en este ciclo
      G_phantomBuyDetected = true;

      // Marcador visual de BUY fantasma
      string nameBuy = "OHL_PHANTOM_BUY_" + Symbol() + "_" + IntegerToString(MagicNumber) + "_" + TimeToString(TimeCurrent(), TIME_SECONDS);
      ObjectCreate(nameBuy, OBJ_ARROW, 0, TimeCurrent(), candidateBuyPrice);
      ObjectSetInteger(0, nameBuy, OBJPROP_COLOR, clrBlue);
      ObjectSetInteger(0, nameBuy, OBJPROP_ARROWCODE, 233); // Flecha sólida hacia abajo para BUY
      ObjectSetInteger(0, nameBuy, OBJPROP_WIDTH, 3);
      ObjectSetInteger(0, nameBuy, OBJPROP_BACK, false);
      ObjectSetInteger(0, nameBuy, OBJPROP_SELECTABLE, false);
      // CORREGIDO: Guardar precio en CADA intento (fantasma o real)
      GlobalVariableSet(gvLastExecKey, candidateBuyPrice);

      return(false);
     }

   int TicketNuevoCompra = OrderSend(NULL, OP_BUY, lots, candidateBuyPrice, 10, 0, 0, NULL, MagicNumber, 0, clrGreen);
   if(TicketNuevoCompra < 0)
     ...
     {
      // ✅ Orden ejecutada exitosamente - AHORA SÍ actualizamos el precio de referencia
      GlobalVariableSet(gvLastExecKey, candidateBuyPrice);
      ...
      ImprimirAnalisisIndicadores("BUY", TicketNuevoCompra, lots, candidateBuyPrice);
...
// PERO solo si MyMartingala() no ya calculó un lotaje mayor (indicando que perdió con martingala aplicada)
   double lotajeBaseActual = CalcularLotajeBase();
// Verificar si MyMartingala() ya calculó un lotaje mayor (cuando perdió con martingala aplicada)
// IMPORTANTE: MyMartingala() devuelve lots, así que verificamos lots también para Venta
   bool yaCalculoMartingala = (lots > lotajeBaseActual || lots > lotajeBaseActual);
// Verificar si MyMartingala() ya devolvió un lotaje mayor que el base (indicando que ya aplicó martingala)
// O si estamos en recuperación y necesitamos aplicar martingala
   bool necesitaAplicarMartingala = (!enRachaMala && lotajePerdidoGuardado > 0 && !yaCalculoMartingala);
   if(necesitaAplicarMartingala)
     {
      ...
      LogEnPantalla("✓✓ Aplicando martingala SELL - Lotaje: " + DoubleToString(lots, 2));
...
// GATING DE PRECIO: solo ejecutar SELL si el precio va MÁS BAJO que la última operación (detección de tendencia bajista)
...
// SELL debe ser MÁS BAJO que la última operación (sea BUY o SELL)
   bool sellAllowed = (lastExecPrice == 0.0) || (candidateSellPrice < lastExecPrice);
   LogEnPantalla(">>> DEBUG SELL: Precio actual=" + DoubleToString(candidateSellPrice, Digits) + " | Ultimo precio=" + DoubleToString(lastExecPrice, Digits) + " | Permitido=" + (sellAllowed ? "SI" : "NO (FANTASMA)"));

// NUEVO: Si ya hubo una orden fantasma SELL en este ciclo, bloquear hasta que se resetee la MEGA-BANDERA
   if(G_phantomSellDetected && G_megaSellFlag)
     {
      LogEnPantalla("⛔ SELL bloqueado - Esperando nuevo ciclo MEGA");
      return(false);
     }
   ...
   if(!sellAllowed)
     {
      ...
      // MARCAR que se detectó una orden fantasma en este ciclo
      G_phantomSellDetected = true;

      // Marcador visual de SELL fantasma
      string nameSell = "OHL_PHANTOM_SELL_" + Symbol() + "_" + IntegerToString(MagicNumber) + "_" + TimeToString(TimeCurrent(), TIME_SECONDS);
      ObjectCreate(nameSell, OBJ_ARROW, 0, TimeCurrent(), candidateSellPrice);
      ObjectSetInteger(0, nameSell, OBJPROP_COLOR, clrRed);
      ObjectSetInteger(0, nameSell, OBJPROP_ARROWCODE, 234); // Flecha sólida hacia arriba para SELL
      ObjectSetInteger(0, nameSell, OBJPROP_WIDTH, 3);
      ObjectSetInteger(0, nameSell, OBJPROP_BACK, false);
      ObjectSetInteger(0, nameSell, OBJPROP_SELECTABLE, false);
      // CORREGIDO: Guardar precio en CADA intento (fantasma o real)
      GlobalVariableSet(gvLastExecKey, candidateSellPrice);
      return(false);
     }

   int TicketNuevoVenta = OrderSend(NULL, OP_SELL, lots, candidateSellPrice, 10, 0, 0, NULL, MagicNumber, 0, clrRed);
   ...
   if(TicketNuevoVenta < 0)
   ...
   else
     {
      ...
      // NUEVO: Imprimir análisis de indicadores técnicos
      ImprimirAnalisisIndicadores("SELL", TicketNuevoVenta, lots, candidateSellPrice);
