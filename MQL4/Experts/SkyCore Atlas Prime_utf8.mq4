...
//═══════════════════════════════════════════════════════════════════
//  SKYCORE ATLAS PRIME v2.0 - Single Martingale Recovery System
//  Conservative approach with one recovery attempt
//═══════════════════════════════════════════════════════════════════
...
extern int MaxSpread = 90; // Maximum allowed spread for operations
input int MagicNumber = 555; // Unique Robot ID (change if running multiple instances)

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  2. ENTRY STRATEGY (Signals & Confirmations)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __ENTRY__ = "════ ENTRY SIGNALS ════";
input bool banderaEstocastico = false; // Use Stochastic flag (first confirmation)
input bool restriccionMedia200 = false; // Use MA200 restriction (BUY above, SELL below)
input bool usarMegaComoGatillo = true; // Use MEGA-FLAG as trigger (disables MA8xMA5 crossover)
...
input int barrasParaPromedio = 20; // Number of bars for average height calculation
input double tpMultiplicador = 1.0; // TP = X times bar average height
input double slMultiplicador = 3.0; // SL = X times bar average height (Recommended: 3.0)

input string __RISK_NOTE__ = "⚠ If dynamic TP/SL is ON, fixed values below are IGNORED";

// Fixed TP/SL (Used only if dynamic is OFF)
bool useTP = true; // Use fixed TP?
input double TP = 1500.0; // Fixed Take Profit (points) - used only if dynamic is OFF
bool useSL = true; // Use fixed SL?
...
bool useInteresCompuesto = true; // Enable Compound Interest
input double capitalBase = 3000.0; // Balance needed for 0.01 lots (Higher = more conservative)
input double lotsPorCapital = 0.01; // Lot size per capital base unit
double lotajeDeseado = 0.01; // Initial lot size (calculated automatically with compound interest)

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  5. MARTINGALE RECOVERY SYSTEM (Atlas: 1 Attempt)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __MARTINGALE__ = "════ MARTINGALE SYSTEM ════";
bool martingala = true; // Enable Martingale recovery
input double myMultiplier = 4.0; // Multiplier for recovery attempt (Recommended: 3.0-5.0)
input int tipoMartingala = 1; // Martingale type: 0=Classic | 1=Bad Streak System
input bool hacerMartingalaAcumulativo = false; // Cumulative Martingale: cover ALL accumulated losses
int cuandoUsarMartingala = 1; // Max martingale attempts (Atlas: always 1)
input int maxLost = 20; // Maximum consecutive losses before stopping robot
double porcentajeExtra = 0.1; // % Extra profit required

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  6. EXIT STRATEGY (Profit Targets & Stop Management)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __EXIT__ = "════ EXIT STRATEGY ════";

// Trailing Stop
input bool useTS = true; // Enable Trailing Stop
input double minDistanceToActivateTS = 10000.0; // Minimum pips to activate Trailing Stop
input double trailingStopBuffer = 6000.0; // Buffer in pips before moving the stop
input double initialTSMove = 6000.0; // Pips that SL moves initially

// Breakeven
input bool useBreakevenStop = true; // Enable Breakeven Stop (move SL to entry)
input double breakevenDistance = 3000.0; // Distance in pips to activate breakeven
input double breakevenLossPips = 1000.0; // Small loss in pips for breakeven (e.g., -1000 = small loss)

// MA200 Exit
input bool useMA200Exit = false; // Close position if bar closes on opposite side of MA200

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  7. SCALPING & QUICK EXITS
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __SCALPING__ = "════ SCALPING MODE ════";
input bool usarScalpingAgresivo = false; // Enable aggressive scalping
input int gananciaMinimaTics = 3000; // Minimum profit in ticks to close
input int gananciaRapidaTics = 1500; // Quick profit in ticks for fast exit
bool cerrarEnPrimeraGanancia = true; // Close on first minimum profit

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  8. DRAWDOWN PROTECTION
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __PROTECTION__ = "════ PROTECTION SYSTEMS ════";

// Accumulated Loss Protection
input bool proteccionPerdidas = false; // Enable accumulated loss protection
input int perdidasConsecutivasMin = 1; // Activate after N consecutive losses
input double porcentajeExtraCierre = 1.0; // % extra over accumulated losses to close (e.g., 1.0 = 1%)

// Drawdown Protection
input bool usarProteccionDrawdown = true; // Enable drawdown protection
input double maxDrawdownPorcentaje = 50.0; // Maximum allowed drawdown over initial balance (%)
bool cerrarSoloPerdedoras = true; // If true, close only losing positions; if false, close all

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  9. MARKET SAFETY FILTERS (IsMarketSafe)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __FILTERS__ = "════ MARKET FILTERS ════";

// Enable/Disable each filter
input bool usarFiltroEMATrenzada = true; // Filter: Intertwined EMAs
input bool usarFiltroRangoAdaptativo = true; // Filter: Adaptive range
input bool usarFiltroPrecioPegadoEMA = true; // Filter: Price stuck to EMA
input bool usarFiltroSlopeEMA = true; // Filter: EMA slope too flat
input bool usarFiltroAnguloEMA = true; // Filter: EMA angle too shallow
input bool usarFiltroCanalEstrecho = true; // Filter: Narrow channel
input bool usarFiltroAlternanciaVelas = true; // Filter: Candle alternation
input bool usarFiltroMechasLargas = true; // Filter: Long wicks
input bool usarFiltroCuerpoMinimo = true; // Filter: Minimum body size
input bool usarFiltroFaltaContinuacion = true; // Filter: Lack of continuation
input bool usarFiltroRetrocesoInmediato = true; // Filter: Immediate retracement
input bool usarFiltroAceleracionReal = true; // Filter: Real acceleration
input bool mostrarDebugFiltros = false; // Show filter debug messages

// Filter sensitivity parameters
input int filtroLookback = 20; // Lookback period for filters
input int filtroEMATrenzadaMaxCruces = 3; // Maximum EMA crosses
input double filtroRangoMinMultiplicadorATR = 1.8; // Minimum range multiplier (ATR)
input double filtroPrecioPegadoFactorATR = 0.30; // Price stuck factor (ATR)
input double filtroPrecioPegadoMaxRatio = 0.40; // Price stuck maximum ratio
input double filtroSlopeMinFactorATR = 0.20; // Minimum slope factor (ATR)
input double filtroAnguloMinGrados = 3.5; // Minimum angle in degrees
input double filtroCanalFactorATR = 0.35; // Channel factor (ATR)
input double filtroCanalMaxRatioCentral = 0.50; // Maximum central ratio
input double filtroAlternanciaMaxRatio = 0.45; // Maximum alternation ratio
input double filtroMechasFactorCuerpo = 2.0; // Wick factor (body multiplier)
input double filtroCuerpoMinFactorAltura = 0.25; // Minimum body factor (height)
input double filtroContinuacionMinFactorAlt = 0.30; // Minimum continuation factor
input double filtroRetrocesoMaxFactorAlt = 0.40; // Maximum retracement factor
input double filtroAceleracionMinFactorAlt = 0.20; // Minimum acceleration factor

// Horizontal Range Detection
input bool detectarRangoHorizontal = true; // Enable horizontal range detection

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  10. TIME MANAGEMENT (Dangerous Hours)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __TIME__ = "════ TIME FILTERS ════";

// Dangerous Hours (based on historical analysis)
input bool evitarHorariosPeligrosos = true; // Block hours with more historical losses
input int offsetBrokerGMT = 2; // Broker offset from GMT (e.g., GMT+2 → value=2)
input int horaPeligrosa1Inicio = 17; // Dangerous hour 1 start (broker MT4 time)
input int horaPeligrosa1Fin = 20; // Dangerous hour 1 end (broker MT4 time)
input int horaPeligrosa2Inicio = 9; // Dangerous hour 2 start (broker MT4 time)
input int horaPeligrosa2Fin = 10; // Dangerous hour 2 end (broker MT4 time)

// Chaotic Friday Block
input bool bloquearViernesCaotico = true; // Block Friday from 7:30 AM to 7:30 PM (Ecuador time)
input int horaInicioViernes = 12; // Friday block start hour (broker time = 7:30 AM Ecuador)
input int minutoInicioViernes = 30; // Friday block start minute
input int horaFinViernes = 24; // Friday block end hour (00:00 = midnight, 7:30 PM Ecuador Friday)

//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  11. ADVANCED SETTINGS (Experimental)
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
input string __ADVANCED__ = "════ ADVANCED ════";
input bool ajusteEntradaStochastic = false; // EXPERIMENT: Close and reopen positions based on Stochastic
input bool usarCierrePorEstocastico = false; // Enable position closing by Stochastic (overbought/oversold)

//═══════════════════════════════════════════════════════════════════
//  INTERNAL VARIABLES (Do not modify)
//═══════════════════════════════════════════════════════════════════
double metaDiaria = 0; // Daily target
int CantOperacionesMax = 1; // Max number of simultaneous operations
bool activarParidad = true;
int cantOpe = 1; // Number of operations per pair

int OBLevel = 50; // Medium level - for sells (stoch > 50 with negative slope)
int OSLevel = 50; // Medium level - for buys (stoch < 50 with positive slope)
bool usarIA = false; // Use AI
double tpPercent = 1.5; // TP proportion
double slPercent = 0.5; // SL proportion
int VelasContra = 8; // Candles to wait before evaluating
double DojiRatio = 0.25; // Maximum % of body vs total range to consider doji
double RechazoMinimo = 1.1; // Minimum % of wick in favor of movement
double extraSlSpace = 0; // Extra space after SL
int countTS = 102; // Max for TS
double datatrailingStopPercentage = 0.5; // Trailing Stop percentage
double N = 1; // Entry level
double myVela = 1; // Space after SL
int slippage = 3; // Maximum slippage in points for closes/opens
bool usarRestriccionVelaGrande = true; // Use large candle restriction

// Trend Filters (Internal)
                  ...
                  if(enRachaMala && prevLots <= lotajeMinimo)
                    {
                     ...
                     // Esperamos a que la siguiente operación ganadora (de cualquier tipo) aplique el multiplicador
   ...
   if(contadorPerdidas() > maxLost)
     {
      ...
      LogEnPantalla("Pérdidas máximas alcanzadas. Reset completo. Lotaje: "+ DoubleToString(lotsBuy, 2));
            ...
            if(OrderProfit() > 0)
              {
               ...
               if(enRachaMala && prevLots <= lotajeMinimo)
               ...
               else
                 {
                  ...
                  if(prevLots >= lotajeBaseActual && tipoOperacionGanadora != -1)
                    {
                     lotsBuy = lotajeBaseActual;
                     lotajePerdidoGuardado = 0.0;
                     enRachaMala = false;
               ...
               // Verificar margen
               double requiredMargin = MarketInfo(Symbol(), MODE_MARGINREQUIRED) * lotsBuy;
            ...
            // CASO 2: Operación PERDEDORA
            else
              {
               if(!martingala)
                 {
                  lotsBuy = lotajeBaseActual;
                  LogEnPantalla("Martingala desactivada. Lotaje: "+ DoubleToString(lotsBuy, 2));
                 }
               else
                 {
                  // Si acabamos de perder con martingala aplicado (lote mayor al base)
                  if(prevLots > lotajeBaseActual)
                    {
                     ...
                     LogEnPantalla("Perdió con martingala. Guardando lotaje: "+ DoubleToString(prevLots, 2) +
                                   ". Entrando en racha mala con: "+ DoubleToString(lotsBuy, 2));
                    }
                  // Si estamos en racha mala
                  else
                     if(enRachaMala)
                       {
                        // Seguimos operando con lotaje mínimo
                        lotsBuy = lotajeMinimo;
                        LogEnPantalla("En racha mala. Operando con: "+ DoubleToString(lotsBuy, 2));
                       }
                     // Primera pérdida con lotaje normal
                     else
                       {
   ...
   if(lotsBuy == 0.0)
     {
      ...
      enRachaMala = false;
      perdidasAcumuladas = 0.0;  // NUEVO: Reset perdidas acumuladas
      LogEnPantalla("Primera orden. Lotaje: "+ DoubleToString(lotsBuy, 2));
     }
   ...
   if(contadorPerdidas() > maxLost)
     {
      lotsBuy = lotajeBaseActual;
      numeroDeIntentos = 0;
      LogEnPantalla("Pérdidas máximas alcanzadas (Clásica). Reset. Lotaje: "+ DoubleToString(lotsBuy, 2));
      lotsBuy = NormalizeDouble(lotsBuy / lotstep, 0) * lotstep;
      if(lotsBuy < minlot)
         lotsBuy = minlot;
      if(lotsBuy > MaximalLots)
            ...
            if(OrderProfit() > 0)
              {
               ...
               lotsBuy = lotajeBaseActual;
               numeroDeIntentos = 0;
               LogEnPantalla("Ganancia (Clásica). Reset. Lotaje: "+ DoubleToString(lotsBuy, 2));
            ...
            // CASO 2: Operación PERDEDORA
            else
              {
               if(!martingala)
                 {
                  lotsBuy = lotajeBaseActual;
                  LogEnPantalla("Martingala desactivada (Clásica). Lotaje: "+ DoubleToString(lotsBuy, 2));
                 }
               else
                 {
                  ...
                  if(numeroDeIntentos < maxIntentos)
                    {
                     numeroDeIntentos++;
                     lotsBuy = prevLots * myMultiplier;
                     LogEnPantalla("Pérdida (Clásica). Multiplicando: "+ DoubleToString(prevLots, 2) + " x " + DoubleToString(myMultiplier, 2) + " = " + DoubleToString(lotsBuy, 2));
                    }
                  else
                    {
                     ...
                     lotsBuy = lotajeBaseActual;
                     numeroDeIntentos = 0;
                     LogEnPantalla("Máx intentos alcanzados (Clásica). Reset. Lotaje: "+ DoubleToString(lotsBuy, 2));
                    }
                 }
   ...
   if(lotsBuy == 0.0)
     {
      lotsBuy = lotajeBaseActual;
      numeroDeIntentos = 0;
      LogEnPantalla("Primera orden (Clásica). Lotaje: "+ DoubleToString(lotsBuy, 2));
     }
...
// Inicializar lotaje con el valor calculado por la martingala
   lotsBuy = SeleccionarMartingala();





   if(enRachaMala && lotajePerdidoGuardado > 0 && tipoOperacionGanadora != -1)
     {
      ...
      // Aplicar multiplicador a la siguiente operación ganadora (de cualquier tipo)
      // MODO ACUMULATIVO: Calcular lotaje que cubra perdidas acumuladas
      if(hacerMartingalaAcumulativo && perdidasAcumuladas > 0)
        {


         // NUEVO: Buscar la ganancia de la operación con 0.01 para restarla
         ...
         for(int x = OrdersHistoryTotal() - 1; x >= 0; x--)
           {
            if(OrderSelect(x, SELECT_BY_POS, MODE_HISTORY))
              {
               if(OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber &&
                  OrderType() == tipoOperacionGanadora && OrderLots() <= lotajeMinimo &&
                  OrderProfit() > 0)
                 {
      ...
      else
        {
         ...
         lotsBuy = lotajePerdidoGuardado * myMultiplier;
        }
   ...
   if(!buyAllowed)
     {
      ...
      ObjectSetInteger(0, nameBuy, OBJPROP_COLOR, clrBlack);
      ...
      Alert("PHANTOM BUY (", Symbol(), ") última op=", DoubleToString(lastExecPrice, Digits),
            " intento=", DoubleToString(candidateBuyPrice, Digits));
...
// Inicializar lotaje con el valor calculado por la martingala
   lotsSell = SeleccionarMartingala();
   ...
   if(enRachaMala && lotajePerdidoGuardado > 0 && tipoOperacionGanadora != -1)
     {
      ...
      if(hacerMartingalaAcumulativo && perdidasAcumuladas > 0)
      ...
      else
        {
         ...
         // MODO NORMAL: Multiplicador
         lotsSell = lotajePerdidoGuardado * myMultiplier;
   ...
   if(!sellAllowed)
     {
      ...
      ObjectSetInteger(0, nameSell, OBJPROP_COLOR, clrBlack);
      ...
      Alert("PHANTOM SELL (", Symbol(), ") última op=", DoubleToString(lastExecPrice, Digits),
            " intento=", DoubleToString(candidateSellPrice, Digits));
      // CORREGIDO: Guardar precio en CADA intento (fantasma o real)
